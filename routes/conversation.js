...
router.get('/me', auth, authorizeRoles('user', 'groupAdmin', 'admin'), conversationController.getMyConversations);

// @route   GET /api/conversations/:chatIdentifier/messages
// @desc    Get messages for a specific private conversation
router.get('/:chatIdentifier/messages', auth, authorizeRoles('user', 'groupAdmin', 'admin'), conversationController.getConversationMessages);

// @route   PUT /api/conversations/:chatIdentifier/read
// @desc    Mark messages in a private conversation as read by the current user
router.put('/:chatIdentifier/read', auth, authorizeRoles('user', 'groupAdmin', 'admin'), conversationController.markConversationAsRead);
