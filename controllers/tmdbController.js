...
const handleTmdbError = (res, err, message) => {
  ...
  res
    .status(500)
    .json({ error: `Server Error while ${message.toLowerCase()}` });
};
...
exports.searchTmdb = async (req, res) => {
  ...
  if (!["movie", "tv", "multi"].includes(type)) {
    return res
      .status(400)
      .json({ error: 'Invalid type. Must be "movie", "tv", or "multi".' });
  }
  try {
...
exports.getTmdbTrending = async (req, res) => {
  ...
  if (!["day", "week"].includes(time_window)) {
    return res
      .status(400)
      .json({ error: 'Invalid time window. Must be "day" or "week".' });
  }
  try {
    const tmdbType = type === "all" ? "all" : type;
    const response = await axios.get(
      `${TMDB_BASE_URL}/trending/${type}/${time_window}`,
      {
        params: {
          api_key: TMDB_API_KEY,
        },
