    ...
    if (profilePicture === null || profilePicture === "") {
      ...
      if (!isValidAvatar(profilePicture)) {
        return res.status(400).json({
          success: false,
          message:
            "Invalid profile picture. Please select from available avatars.",
        });
      }
...
exports.updateProfileSettings = async (req, res) => {
  try {
    ...
    if (showWatchedContent !== undefined)
      user.profileSettings.showWatchedContent = showWatchedContent;
    if (showFavorites !== undefined)
      user.profileSettings.showFavorites = showFavorites;
...
// -- ROUTES FOR MANAGING MOVIE FAVORITES --
...
exports.addFavoriteGenre = async (req, res) => {
  ...
  try {
    ...
    if (
      user.favoriteGenres.map((g) => g.toLowerCase()).includes(normalizedGenre)
    ) {
...
module.exports = { ...exports, ...watchlistController };
