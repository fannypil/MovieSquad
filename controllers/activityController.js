...
exports.getMyPostsActivity = async (req, res) => {
  try {
    const myPosts = await Post.find({ author: req.user.id })
      ...
      .limit(20)
      .select(
        "content tmdbTitle tmdbType tmdbPosterPath groupId createdAt likes comments"
      );
...
exports.getUserPostsActivity = async (req, res) => {
  try {
    ...
    const userPosts = await Post.find({ author: userId })
      ...
      .limit(20)
      .select(
        "content tmdbTitle tmdbType tmdbPosterPath group createdAt likes comments"
      ); // Fixed: changed 'groupId' to 'group'
