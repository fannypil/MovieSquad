...
exports.getMyConversations = async (req, res) => {
    try {
        ...
        const conversations = await Promise.all(uniqueChatIdentifiers.map(async (identifier) => {
            const lastMessage = await Message.findOne({ chatIdentifier: identifier })
            ...
            const otherParticipantId = lastMessage.sender._id.toString() === userId ? lastMessage.recipient._id : lastMessage.sender._id;
            const otherParticipant = await User.findById(otherParticipantId).select('username profilePicture').lean();
