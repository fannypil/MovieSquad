...
exports.getPostsPerGroupMonthly = async (req, res) => {
    try {
        ...
        handleServerError(res, error, 'Error retrieving posts per group monthly data');
    }
};
...
exports.getTopGenresByUserCount = async (req, res) => {
    try {
        ...
        const formattedResults = results.map((item, index) => ({
            ...
            percentage: totalUsersWithGenres > 0 ? 
                Math.round((item.userCount / totalUsersWithGenres) * 100 * 100) / 100 : 0,
...
exports.getPostsPerUserMonthly = async (req, res) => {
    try {
        ...
        handleServerError(res, error, 'Error retrieving posts per user monthly data');
    }
};
...
exports.getStatsSummary = async (req, res) => {
    try {
        ...
        const activeUserCount = activeUsers.length > 0 ? activeUsers[0].activeUsers : 0;
