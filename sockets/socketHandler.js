...
module.exports = (io) => {
  ...
  io.on("connection", (socket) => {
    console.log(
      `User connected: ${socket.user.username} (${socket.user.id}) - Socket ID: ${socket.id}`
    );
    //join the user to their own private room for direct messages
    socket.join(socket.user.id.toString());
    console.log(
      `${socket.user.username} joined their personal room: ${socket.user.id}`
    );
    // Group Chat Events
    socket.on("joinGroup", async (groupId) => {
      ...
      try {
        ...
        if (
          ...
          socket.emit(
            "groupError",
            "You are not a member of this group or do not have access."
          );
          return;
        }
        socket.join(groupId);
        console.log(
          `${socket.user.username} joined group: ${group.name} (${groupId})`
        );
        // Emit a success message back to the client that joined
        socket.emit("joinedGroup", {
          groupId: group._id,
          groupName: group.name,
          msg: `You have joined "${group.name}" chat.`,
        });
    ...
    socket.on("sendGroupMessage", async ({ groupId, content }) => {
      if (!groupId || !content) {
        return socket.emit(
          "chatError",
          "Group ID and message content are required."
        );
      }
      try {
        ...
        if (
          ...
          return socket.emit(
            "chatError",
            "You are not authorized to send messages to this group."
          );
        }
        ...
        console.log(
          `Message sent to  group ${groupId} by ${socket.user.username}: ${content}`
        );
      } catch (error) {
        console.error(
          `Error sending message to group ${groupId}:`,
          error.message
        );
    ...
    socket.on("joinPrivateChat", async (otherUserId) => {
      ...
      if (!otherUserId || currentUserId === otherUserId) {
        return socket.emit(
          "privateChatError",
          "Invalid recipient for private chat."
        );
      }
      ...
      try {
        ...
        if (
          !currentUser.friends.includes(otherUserId) &&
          socket.user.role !== "admin"
        ) {
          return socket.emit(
            "privateChatError",
        ...
        const chatIdentifier = getPrivateChatIdentifier(
          currentUserId,
          otherUserId
        );
        socket.join(chatIdentifier); // Join the unique room for this 1-on-1 chat
        console.log(
          `${socket.user.username} joined private chat with ${otherUser.username} in room: ${chatIdentifier}`
        );
        ...
        socket.emit("privateChatHistory", {
          chatIdentifier,
          messages: privateChatHistory,
        });
      } catch (error) {
        console.error(
          `Error joining private chat with ${otherUserId}:`,
          error.message
        );
    ...
    socket.on("sendPrivateMessage", async ({ recipientId, content }) => {
      ...
      try {
        ...
        if (
          ...
          return socket.emit(
            "privateChatError",
            "You can only send private messages to friends."
          );
        }
        ...
        console.log(
          `Private message sent from ${socket.user.username} to ${recipientId}: ${content}`
        );
        ...
        const isInChatRoom =
          recipientSocket && recipientSocket.rooms.has(chatIdentifier);

        if (socket.user._id.toString() !== recipientId && !isInChatRoom) {
          await createNotification(recipientId, "new_private_message", {
    ...
    socket.on("stopTyping", ({ groupId, recipientId }) => {
      if (groupId) {
        ...
        console.log(
          `⌨️ ${socket.user.username} stopped typing to ${recipientId} in ${chatIdentifier}`
        );
      }
    });
    ...
    socket.on("disconnect", () => {
      console.log(
        `User disconnected: ${socket.user.username} (${socket.user.id}) - Socket ID: ${socket.id}`
      );
    });
  });
};
