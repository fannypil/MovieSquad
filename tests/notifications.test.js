...
describe('🔔 Notification System Tests', () => {
    ...
    describe('📝 Post Interaction Notifications', () => {
        ...
        test('Should not create notification when user likes their own post', async () => {
            ...
            const selfLikeNotification = res.body.find(n => 
                n.type === 'like' && n.sender._id === user1Id
            );
    ...
    describe('👥 Friend Request Notifications', () => {
        beforeEach(async () => {
            // Clear notifications
            await Notification.deleteMany({ recipient: { $in: [user1Id, user2Id, user3Id] } });
        });
        ...
        test('Should create notification when user sends friend request', async () => {
            ...
            const friendRequestNotification = res.body.find(n => n.type === 'friend_request');
            expect(friendRequestNotification).toBeDefined();
            expect(friendRequestNotification.sender.username).toBe('regular_user');
            expect(friendRequestNotification.message).toContain('sent you a friend request');
        });
        ...
        test('Should create notification when friend request is accepted', async () => {
            ...
            const acceptedNotification = res.body.find(n => n.type === 'friend_accepted');
            expect(acceptedNotification).toBeDefined();
            expect(acceptedNotification.sender.username).toBe('regular_user');
            expect(acceptedNotification.message).toContain('accepted your friend request');
        });
    });
    ...
    describe('🎯 Group Invitation Notifications', () => {
        ...
        test('Should create notification when user is invited to private group', async () => {
            ...
            const inviteNotification = res.body.find(n => n.type === 'group_invite');
        ...
        test('Should create notification when user requests to join private group', async () => {
            ...
            const requestNotification = res.body.find(n => n.type === 'group_join_request');
            expect(requestNotification).toBeDefined();
            expect(requestNotification.sender.username).toBe('third_user');
            expect(requestNotification.message).toContain('requested to join your group');
        });
        ...
        test('Should create notification when join request is accepted', async () => {
            ...
            const acceptNotification = res.body.find(n => n.type === 'group_request_accepted');
            expect(acceptNotification).toBeDefined();
            expect(acceptNotification.message).toContain('request to join the group was accepted');
        });
        ...
        test('Should create notification when join request is rejected', async () => {
            ...
            const rejectNotification = res.body.find(n => n.type === 'group_request_rejected');
            expect(rejectNotification).toBeDefined();
            expect(rejectNotification.message).toContain('request to join the group was rejected');
        });
        ...
        test('Should create notification when user is removed from group', async () => {
            ...
            const inviteNotification = inviteNotificationRes.body.find(n => n.type === 'group_invite');
            ...
            console.log('Group members:', groupRes.body.members.map(m => ({ id: m._id, username: m.username })));
            ...
            const removeNotification = res.body.find(n => n.type === 'group_removed');
   ...
   describe('🔧 Notification Management', () => {
    ...
    beforeEach(async () => {
        ...
        if (notificationsRes.body.length === 0) {
            console.warn('No notifications created despite successful like operation');
    ...
    test('Should delete a notification', async () => {
        ...
        const deletedNotification = notificationsRes.body.find(n => n._id === testNotificationId);
