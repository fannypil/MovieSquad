...
const isValidAvatar = (profilePicture) => {
    ...
    return PREDEFINED_AVATARS.some(avatar => 
        avatar.url === profilePicture || avatar.id === profilePicture
    );
};
...
const getAvatarUrl = (avatarIdOrUrl) => {
    ...
    const avatar = PREDEFINED_AVATARS.find(avatar => avatar.id === avatarIdOrUrl);
