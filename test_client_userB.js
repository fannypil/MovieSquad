...
const userA_Credentials = {
  // These are now User B's credentials
...
async function startClient(
  clientName,
  userCredentials,
  otherUserIdForPrivateChat = null,
  groupIdForGroupChat = null
) {
  try {
    ...
    console.log(
      `${clientName}: Logged in as ${loggedInUser.username} (${loggedInUser._id}). Token: ${token.substring(0, 15)}...`
    );
    ...
    socket.on("connect", () => {
      console.log(
        `--- ${clientName}: Socket.io connected: ${socket.id} for user ${loggedInUser.username}`
      );

      // --- Group Chat Test (Optional, if groupIdForGroupChat is provided) ---
      if (
        groupIdForGroupChat &&
        groupIdForGroupChat !== "PASTE_YOUR_GROUP_ID_HERE"
      ) {
        console.log(
          `${clientName}: Attempting to join group: ${groupIdForGroupChat}`
        );
      ...
      if (
        otherUserIdForPrivateChat &&
        otherUserIdForPrivateChat !== "USER_B_MONGO_ID_HERE"
      ) {
        console.log(
          `${clientName}: Attempting to join private chat with user ID: ${otherUserIdForPrivateChat}`
        );
    ...
    socket.on("connect_error", (err) => {
      console.error(
        `--- ${clientName}: Socket.io Connection Error:`,
        err.message
      );
    });
    ...
    socket.on("joinedGroup", (data) => {
      ...
      setTimeout(() => {
        ...
        setTimeout(
          () => socket.emit("stopTyping", { groupId: data.groupId }),
          2000
        );
      }, 1000);
    });
    ...
    socket.on("chatHistory", (data) => {
      ...
      if (data.messages.length > 0) {
        data.messages.forEach((msg) => {
          console.log(
            `  [${new Date(msg.createdAt).toLocaleTimeString()}] ${msg.sender.username}: ${msg.content}`
          );
        });
      } else {
    ...
    socket.on("groupMessage", (message) => {
      if (message.sender._id !== loggedInUser._id) {
        // Avoid logging own sent message twice
        console.log(
          `--- ${clientName}: New Group Message from ${message.sender.username} in group ${message.group}: "${message.content}"`
        );
      }
    });
    ...
    socket.on("joinedPrivateChat", (data) => {
      console.log(
        `--- ${clientName}: Successfully joined private chat with ${data.otherUser.username} (${data.otherUser._id})`
      );
      ...
      setTimeout(() => {
        ...
        console.log(
          `${clientName}: Sending private message: "${messageContent}" to ${data.otherUser.username}`
        );
        socket.emit("sendPrivateMessage", {
          recipientId: data.otherUser._id,
          content: messageContent,
        });
        socket.emit("typing", { recipientId: data.otherUser._id });
        setTimeout(
          () => socket.emit("stopTyping", { recipientId: data.otherUser._id }),
          2000
        );
      }, 2000); // Small delay
    });
    ...
    socket.on("privateChatHistory", (data) => {
      console.log(
        `--- ${clientName}: Private chat history with ${data.chatIdentifier}:`
      );
      if (data.messages.length > 0) {
        data.messages.forEach((msg) => {
          console.log(
            `  [${new Date(msg.createdAt).toLocaleTimeString()}] ${msg.sender.username} -> ${msg.recipient.username}: ${msg.content}`
          );
        });
      } else {
    ...
    socket.on("privateMessage", (message) => {
      ...
      const currentChatIdentifier = getPrivateChatIdentifier(
        loggedInUser._id,
        message.recipient._id.toString() === loggedInUser._id.toString()
          ? message.sender._id.toString()
          : message.recipient._id.toString()
      );
      if (
        message.chatIdentifier === currentChatIdentifier &&
        message.sender._id !== loggedInUser._id
      ) {
        console.log(
          `--- ${clientName}: New Private Message from ${message.sender.username}: "${message.content}"`
        );
      }
    });
    ...
    socket.on("typing", (data) => {
      if (data.userId !== loggedInUser._id) {
        // Only log if it's not our own typing
        const chatType = data.groupId
          ? `group ${data.groupId}`
          : `private chat ${data.chatIdentifier}`;
        console.log(
          `--- ${clientName}: ${data.username} is typing in ${chatType}...`
        );
      }
    });
    ...
    socket.on("stopTyping", (data) => {
      if (data.userId !== loggedInUser._id) {
        const chatType = data.groupId
          ...
          : `private chat ${data.chatIdentifier}`;
        console.log(
          `--- ${clientName}: ${data.username} stopped typing in ${chatType}.`
        );
      }
    });
    ...
    if (error.response) {
      console.error(
        `--- ${clientName}: Login Error Response:`,
        error.response.data
      );
    }
  }
