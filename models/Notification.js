...
const NotificationSchema = new mongoose.Schema(
  {
    ...
    sender: {
      // Optional: The user who triggered the notification (e.g., who liked, commented, sent request)
      type: mongoose.Schema.Types.ObjectId,
      ref: "User",
      required: false, // Not all notifications have a specific sender (e.g., system notifications)
    },
    type: {
      // Type of notification (e.g., 'like', 'comment', 'friend_request', 'group_invite', 'new_message', 'watchlist_add', 'admin_message')
      type: String,
    ...
    entityId: {
      // Optional: ID of the related entity (e.g., Post ID, Comment ID, Group ID, Message ID)
      type: mongoose.Schema.Types.ObjectId,
      required: false,
      index: true, // Index for direct lookup
    },
    entityType: {
      // Optional: Type of the related entity ('Post', 'Comment', 'Group', 'Message', etc.)
      type: String,
      required: function () {
    ...
    message: {
      // Custom message for the notification (e.g., "John Doe liked your post")
      type: String,
      trim: true,
