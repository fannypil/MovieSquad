...
const MessageSchema = new mongoose.Schema(
  {
    ...
    group: {
      // Optional: If message belongs to a specific group chat
      type: mongoose.Schema.Types.ObjectId,
      ref: "Group",
      required: false,
      index: true,
    ...
    recipient: {
      // Optional: For private messages (1-on-1)
      type: mongoose.Schema.Types.ObjectId,
      ref: "User",
    ...
    chatIdentifier: {
      // For 1-on-1 chats: a unique ID for the conversation between two users
      type: String,
      required: function () {
...
MessageSchema.pre("validate", function (next) {
  if ((this.group && this.recipient) || (!this.group && !this.recipient)) {
    next(
      new Error(
        "A message must either belong to a group or be a private message to a recipient, but not both or neither."
      )
    );
  } else {
    next();
